...
void uop_tmb_shift_imm(struct uop *op);
void uop_tmb_shift_imm(struct uop *op)
{
    ...
    CPU_TRACE(5, "\t\tuop_tmb_shift_imm: opcode %d, immed %d, Rm %d, Rd %d\n",
              opcode, immed, Rm, Rd);
...
void uop_tmb_add_sub_3reg(struct uop *op);
void uop_tmb_add_sub_3reg(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int Rn, Rd;
    ...
    if (BIT(ins, 10)) {
        ...
        CPU_TRACE(5, "\t\tuop_tmb_add_sub_3reg: sub %d, Rd %d, Rn %d, immediate %d\n",
        ...
        CPU_TRACE(5, "\t\tuop_tmb_add_sub_3reg: sub %d, Rd %d, Rn %d, Rm %d\n",
                  opcode, Rd, Rn, Rm);
...
void uop_tmb_add_sub_large_immediate(struct uop *op);
void uop_tmb_add_sub_large_immediate(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int RdRn;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_add_sub_large_immediate: opcode %d, Rd/Rn %d, immediate %d\n",
...
void uop_tmb_data_processing(struct uop *op);
void uop_tmb_data_processing(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int RmRs, RdRn;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_data_processing: opcode %d, RmRs %d, RdRn %d\n", opcode, RmRs, RdRn);
...
void uop_tmb_special_data_processing(struct uop *op);
void uop_tmb_special_data_processing(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int RdRn, Rm;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_special_data_processing: opcode %d, Rd/Rn %d, Rm %d\n",
              opcode, RdRn, Rm);
...
void uop_tmb_literal_load(struct uop *op);
void uop_tmb_literal_load(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int Rd;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_literal_load: Rd %d, immed %d, addr 0x%x\n",
              Rd, immed, addr);
...
void uop_tmb_load_store_register(struct uop *op);
void uop_tmb_load_store_register(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int opcode;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_load_store_register: op %d Rd %d, Rm %d, Rn %d\n",
              opcode, Rd, Rm, Rn);
}

void uop_tmb_load_store_immediate_offset(struct uop *op);
void uop_tmb_load_store_immediate_offset(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int Rn, Rd;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_load_store_immediate: size %d, L %d, Rn %d, Rd %d, immed %d\n",
              size, L, Rn, Rd, immed);
}

void uop_tmb_load_store_from_stack(struct uop *op);
void uop_tmb_load_store_from_stack(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int Rd;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_load_store_from_stack: L %d, Rd %d, immed %d\n",
              L, Rd, immed);
...
void uop_tmb_load_store_multiple(struct uop *op);
void uop_tmb_load_store_multiple(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int L;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_load_store_multiple: L %d Rn %d reg_list 0x%x\n", L?1:0, Rn, reg_list);
...
void uop_tmb_add_to_sp_pc(struct uop *op);
void uop_tmb_add_to_sp_pc(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int Rd;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_add_to_sp_pc: SP %d, Rd %d, immed %d\n", sp?1:0, Rd, immed);
    ...
    if (sp) {
        ...
        panic_cpu("uop_tmb_add_to_sp_pc unimplemented form\n");
        put_reg(Rd, (get_reg(15) & 0xfffffffc) + immed);
    }
}

void uop_tmb_adjust_sp(struct uop *op);
void uop_tmb_adjust_sp(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int sub;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_adjust_sp: sub %d, immed %d\n", sub?1:0, immed_7);
...
void uop_tmb_push_pop_regs(struct uop *op);
void uop_tmb_push_pop_regs(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int L;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_push_pop_regs: L %d R %d reg_list 0x%x, base_offset %d, wb_offset %d\n",
...
void uop_tmb_software_breakpoint(struct uop *op);
void uop_tmb_software_breakpoint(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
...
void uop_tmb_undefined(struct uop *op);
void uop_tmb_undefined(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
...
void uop_tmb_swi(struct uop *op);
void uop_tmb_swi(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
...
void uop_tmb_conditional_branch(struct uop *op);
void uop_tmb_conditional_branch(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int cond;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_conditional_branch: cond 0x%x, immediate %d, target %d\n", cond, immed, target);
...
void uop_tmb_branch(struct uop *op);
void uop_tmb_branch(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    word immed_11;
    ...
    CPU_TRACE(5, "\t\tuop_tmb_branch: immediate %d, target 0x%08x\n", immed_11, target);
...
void uop_tmb_bx_reg(struct uop *op);
void uop_tmb_bx_reg(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    int Rm;
...
void uop_tmb_bl_blx(struct uop *op);
void uop_tmb_bl_blx(struct uop *op)
{
    word ins = op->undecoded.raw_instruction;
    word offset;
...
void thumb_op_load_store_register(struct uop *op);
void thumb_op_load_store_register(struct uop *op)
{
